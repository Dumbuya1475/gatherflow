...
async function getRegisteredEvents(userId: string) {
    ...
    const { data: tickets, error } = await supabase
        .from('tickets')
        .select('events!inner(*, tickets(count), organizer:profiles!events_organizer_id_fkey(first_name, last_name)), id')
        .eq('user_id', userId)
...
async function getPastEvents(userId: string) {
    ...
    const { data: attendedTickets, error: attendedError } = await supabase
        .from('tickets')
        .select('events!inner(*, tickets(count), organizer:profiles!events_organizer_id_fkey(first_name, last_name))')
        .eq('user_id', userId);
