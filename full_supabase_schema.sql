...
CREATE TABLE public.profiles (
    ...
    website text,
    email text UNIQUE,
    is_guest boolean DEFAULT FALSE, -- Added is_guest column
...
CREATE TABLE public.events (
    ...
    ticket_background_image text -- Added ticket_background_image
);
...
CREATE TABLE public.tickets (
    ...
    event_id bigint NOT NULL REFERENCES public.events(id) ON DELETE CASCADE,
    user_id uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
    qr_token uuid UNIQUE, -- Made nullable and removed default
...
BEGIN
    ...
    VALUES (
        ...
        now(),
        now()
    ) ON CONFLICT (email) DO UPDATE SET -- Changed to ON CONFLICT (email)
        id = EXCLUDED.id, -- Update the ID if the email already exists but the ID is different
...
-- Function to generate QR token when ticket is approved
...
-- Trigger to automatically handle QR token generation
